...
export function TrackList({ tracks }: { tracks: Track[] }) {
    ...
    const filteredTracks = tracks.filter(
        (track) => track.title?.toLowerCase().includes(searchQuery.toLowerCase()) ?? track.prompt?.toLowerCase().includes(searchQuery.toLowerCase()),
    );
    ...
    return (
        <div className='flex flex-1 flex-col overflow-y-scroll'>
                    ...
                    {filteredTracks.length > 0 ? (
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className='group relative h-12 w-12 flex-shrink-0 overflow-hidden rounded-md'>
                                                {track.thumbnailUrl ? (
                                                    <img
                                                        src={track.thumbnailUrl} alt="thumbnail song"
                        ...
                        <div className='flex flex-col items-center justify-center pt-20 text-center'>
                            ...
                            <p className='text-muted-foreground mt-1 text-sm'>
                                {searchQuery ? "No track match your search." : "Create your first song to get started."}
                            </p>
                        </div>
                    )}
                </div>
            ...
            {trackToRename && (
                <RenameDialog
                    track={trackToRename}
                    onCloseAction={() => setTrackToRename(null)}
                    onRenameAction={(trackId, newTitle) => renameSong(trackId, newTitle)}
                />
            )}
        </div>
    );
